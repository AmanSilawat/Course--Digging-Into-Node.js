...
async function insertOrLookupOther(other) {
    let result = await SQL3.get(
        ...
        `,
        other
        // data = ?   <--- mean: interpolate value, escaping, quote around stuff
    );
...
async function insertSomething(otherID, something) {
    ...
    if (result && result.changes > 0) { // how many rows changed in db
        return true;
    }
    return false;
}
...
guide:
$ npm i sqlite3
$ node database-and-exercise-setup.js --other=world
*/